...
# --- existing logic (kept, wrapped as functions) ---
...
def get_preview_assets(show: int = 5) -> List[Dict[str, Any]]:
    ...
    out = []
    for a in assets[:show]:
        out.append(extract_asset_info(a))
    return out
...
def cli_main():
    ...
    total = len(assets)
    print(f"🔍 Total assets loaded: {total}")

    if total == 0:
        print("⚠ No assets found!")
        return

    print(f"\n📌 Showing first {args.show} assets:\n")
...
load_dotenv()
API_KEY = os.getenv("API_KEY")  # optional; set in host env for protection
...
def preview(show: int = Query(5, ge=1, le=100), x_api_key: Optional[str] = Header(None)):
    ...
    preview_list = get_preview_assets(show=show)
    return {"total": len(assets), "preview": preview_list}
...
# allow running as script (CLI) or as ASGI app
...
def main():
    ...
    try:
        assets = load_json_file(data_file)
    except Exception as e:
        raise SystemExit(f"❌ Error loading data: {e}")

    print("🚀 ALGO HUNTER ENGINE STARTED")
    print_assets_preview(assets, show=args.show)
    print("\n🎯 Algo Hunter run complete!")
